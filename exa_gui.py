...
def anchor_and_clip(image):
	...
	crop_image = image[y:y + CONFIG["max_window_y"], x:x + CONFIG["max_window_x"]]
	return x, y, crop_image

def read_freecells(image):
	""" Determines how many unlocked free cells there are. For exapunks, this is always one unlocked cell. """
	return [0]
...
def read_stacks(image):
	...
	cards = []
	card_names = []
	for file in glob.glob(CONFIG["card_filename"]):
		card_name = file.rsplit("/", 1)[1].split(".")[0]
		card_names.append(card_name)
		cards.append(cv2.imread(file))

	stacks = []
	for x_stack in range(CONFIG["number_stacks"]):
		stack = []
		for y_stack in range(CONFIG["cards_per_stack_base"]):
			coord_x = CONFIG["base_stack_offset_x"] + (CONFIG["stack_width"] * x_stack)
			coord_y = CONFIG["base_stack_offset_y"] + (CONFIG["stack_height"] * y_stack)
			crop_image = image[coord_y:coord_y + CONFIG["card_sprite_y"], coord_x:coord_x + CONFIG["card_sprite_x"]]

			result_scores = [cv2.matchTemplate(crop_image, cards[i], cv2.TM_SQDIFF) for i in range(len(cards))]
...
def computer_hash(my_image):
	...
	stack_hash = "".join(["S%s/" % "".join([str(s) for s in stack]) for stack in stacks])
...
def execute_solution(offset_x, offset_y, moves):
	...
	pyautogui.mouseDown((offset_x + CONFIG["window_click_offset_x"]) * CONFIG["resolution_scale_click"], (offset_y + CONFIG["window_click_offset_y"]) * CONFIG["resolution_scale_click"], button="left")
	...
	for move in moves:
		...
		if x_pre < CONFIG["number_stacks"]:
			x_pre_final = offset_x + CONFIG["base_stack_offset_x"] + (CONFIG["stack_width"] * x_pre) + CONFIG["click_offset_x"]
			y_pre_final = offset_y + CONFIG["base_stack_offset_y"] + (CONFIG["stack_height"] * y_pre) + CONFIG["click_offset_y"]
		# Separate offsets for freecell
		else:
			x_pre_final = offset_x + CONFIG["freecell_offset_x"] + (CONFIG["stack_width"] * (x_pre - CONFIG["number_stacks"])) + CONFIG["click_offset_x"]
			y_pre_final = offset_y + CONFIG["freecell_offset_y"] + CONFIG["click_offset_y"]

		if x_post < CONFIG["number_stacks"]:
			x_post_final = offset_x + CONFIG["base_stack_offset_x"] + (CONFIG["stack_width"] * x_post) + CONFIG["click_offset_x"]
			y_post_final = offset_y + CONFIG["base_stack_offset_y"] + (CONFIG["stack_height"] * y_post) + CONFIG["click_offset_y"]
		else:
			x_post_final = offset_x + CONFIG["freecell_offset_x"] + (CONFIG["stack_width"] * (x_post - CONFIG["number_stacks"])) + CONFIG["click_offset_x"]
			y_post_final = offset_y + CONFIG["freecell_offset_y"] + CONFIG["click_offset_y"]

		# print("Mouse to %d, %d -> drag to %d, %d" % (x_pre_final, y_pre_final, x_post_final, y_post_final))

		# Move the mouse to the beginning place
		pyautogui.moveTo(x_pre_final * CONFIG["resolution_scale_click"], y_pre_final * CONFIG["resolution_scale_click"], duration = CONFIG["base_delay"])

		# Click and drag to the end
		pyautogui.dragTo(x_post_final * CONFIG["resolution_scale_click"], y_post_final * CONFIG["resolution_scale_click"], duration = CONFIG["base_delay"], button = "left")
...
def click_new_game(offset_x, offset_y):
	...
	pyautogui.mouseDown((offset_x + CONFIG["new_game_offset_x"]) * CONFIG["resolution_scale_click"], (offset_y + CONFIG["new_game_offset_y"]) * CONFIG["resolution_scale_click"], button="left")
	time.sleep(CONFIG["base_delay"] * 3)
	pyautogui.mouseUp()
	time.sleep(CONFIG["base_delay"] * 5)
